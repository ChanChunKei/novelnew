                ...
                <div class="flex items-center gap-3 min-w-0">
                  ...
                  <div class="flex flex-col min-w-0">
                    <div class="flex items-center gap-2">
                      <span v-if="getVolumeTitle(chapter.volume_number)" class="text-xs text-indigo-600 font-medium">
                        {{ getVolumeTitle(chapter.volume_number) }}
                      </span>
...
// 获取卷名
const getVolumeTitle = (volumeNumber: number | undefined): string => {
  if (!volumeNumber) return ''
  const volume = volumes.value.find(v => v.volume_number === volumeNumber)
  return volume?.title || ''
}
