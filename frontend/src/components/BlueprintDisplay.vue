<template>
  <div class="p-8 bg-white rounded-2xl shadow-2xl fade-in">
    ...
    <!-- 加载状态 -->
    <div v-if="isSaving" class="text-center py-8" role="status" aria-live="polite" aria-label="正在保存蓝图">
      <!-- 保存动画 -->
      <div class="relative mx-auto mb-6 w-16 h-16" aria-hidden="true">
        <!-- 旋转圆环 -->
    ...
    <div v-else class="text-center mt-8 space-x-4">
      <button
        ...
        aria-label="重新生成蓝图"
        class="bg-gray-200 text-gray-700 font-bold py-3 px-8 rounded-full hover:bg-gray-300 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-gray-300"
      >
...
const generateBlueprintHTML = () => {
  ...
  const chaptersHTML = (() => {
    ...
    if (chapters.length === 0) {
      return '<p class="text-gray-500 italic">暂无章节大纲</p>'
    }

    // 按卷分组章节
    const chaptersByVolume = new Map<number, typeof chapters>()
    chapters.forEach(ch => {
    ...
    // 按卷号排序
    const sortedVolumes = Array.from(chaptersByVolume.keys()).sort((a, b) => a - b)

    sortedVolumes.forEach(volNum => {
      const volumeChapters = chaptersByVolume.get(volNum)!
      const volume = volumes.find(v => v.volume_number === volNum)
      const volumeTitle = volume?.title || `第${volNum}卷`
      const volumeDesc = volume?.description || ''

      html += `
        <div class="border-2 border-indigo-200 rounded-xl p-6 bg-gradient-to-br from-indigo-50 to-purple-50">
          <!-- 卷标题 -->
          <div class="mb-4 pb-3 border-b-2 border-indigo-300">
            <h3 class="text-2xl font-bold text-indigo-800 flex items-center">
              ...
              ${volumeTitle}
              <span class="ml-3 text-sm font-normal text-indigo-600 bg-white px-3 py-1 rounded-full">共 ${volumeChapters.length} 章</span>
            </h3>
            ${volumeDesc ? `<p class="text-indigo-600 mt-2 ml-8">${volumeDesc}</p>` : ''}
          </div>

          <!-- 章节列表 -->
          <div class="space-y-3">
            ${volumeChapters.map(ch => `
              <div class="group relative overflow-hidden bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-all duration-300">
