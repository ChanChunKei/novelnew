<template>
  <div class="h-screen flex flex-col overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/40">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                ...
                <button
                  ...
                  class="flex-shrink-0 p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all duration-200"
                  title="修改书名"
                >
              ...
              <div class="flex items-center gap-3 mt-0.5">
                ...
                <button
                  v-if="!isAdmin"
                  @click="openEditFanqieIdModal"
                  class="text-xs sm:text-sm text-indigo-600 hover:text-indigo-700 hover:underline flex items-center gap-1"
                  :title="fanqieBookId ? `番茄书籍ID: ${fanqieBookId}` : '设置番茄书籍ID'"
                >
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                  </svg>
                  {{ fanqieBookId ? '已绑定番茄' : '绑定番茄书籍' }}
                </button>
              </div>
            </div>
          </div>
    ...
    <transition
      ...
      leave-to-class="opacity-0 scale-95"
    >
      <div v-if="isEditTitleModalOpen && !isAdmin" class="fixed inset-0 z-50 flex items-center justify-center px-4">
        <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" @click="closeEditTitleModal"></div>
        <div class="relative bg-white rounded-2xl shadow-2xl border border-slate-200 p-6 sm:p-8 w-full max-w-md transform transition-all" @click.stop>
          <h3 class="text-xl font-bold text-slate-900 mb-6">修改书名</h3>
          <div class="space-y-4">
            <div>
    ...
    <transition
      ...
      leave-to-class="opacity-0 scale-95"
    >
      <div v-if="isEditFanqieIdModalOpen && !isAdmin" class="fixed inset-0 z-50 flex items-center justify-center px-4">
        <div class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" @click="closeEditFanqieIdModal"></div>
        <div class="relative bg-white rounded-2xl shadow-2xl border border-slate-200 p-6 sm:p-8 w-full max-w-md transform transition-all" @click.stop>
          <h3 class="text-xl font-bold text-slate-900 mb-4">设置番茄书籍ID</h3>
          <p class="text-sm text-slate-600 mb-6">
            设置后将直接使用此ID上传章节，无需每次查找书籍。<br>
            <span class="text-indigo-600">获取方法：</span>进入番茄小说作家后台 → 章节管理页面 → URL中的数字即为book_id
          </p>
          <div class="space-y-4">
            <div>
              <label for="edit-fanqie-id" class="block text-sm font-semibold text-slate-700 mb-2">
                番茄书籍ID
              </label>
              <input
                id="edit-fanqie-id"
                v-model="editingFanqieId"
                type="text"
                class="block w-full rounded-lg border border-slate-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all duration-200"
                placeholder="例如：7569107762739678233"
                @keyup.enter="saveFanqieId"
              >
            </div>
          </div>
          <div class="mt-6 flex justify-end gap-3">
            <button
              type="button"
              class="px-5 py-2.5 text-sm font-medium text-slate-600 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg transition-all duration-200"
              @click="closeEditFanqieIdModal"
              :disabled="isSavingFanqieId"
            >
              取消
            </button>
            <button
              ...
              class="px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              @click="saveFanqieId"
              :disabled="isSavingFanqieId || !editingFanqieId.trim()"
            >
              {{ isSavingFanqieId ? '保存中...' : '保存' }}
            </button>
          </div>
        </div>
      </div>
...
// Edit Fanqie ID modal state
const isEditFanqieIdModalOpen = ref(false)
const editingFanqieId = ref('')
const isSavingFanqieId = ref(false)
const fanqieBookId = ref<string | null>(null)
...
const loadSection = async (section: SectionKey, force = false) => {
  ...
  try {
    ...
    if (section === 'overview') {
      ...
      fanqieBookId.value = response.data?.fanqie_book_id || null
    }
  } catch (error) {
...
const componentProps = computed(() => {
  ...
  switch (activeSection.value) {
    ...
    case 'chapters':
      return { chapters: data?.chapters || [], volumes: data?.volumes || [], isAdmin: props.isAdmin }
    case 'auto_generator':
...
const openEditTitleModal = () => {
  if (props.isAdmin) return
  editingTitle.value = overviewMeta.title.replace(/^《|》$/g, '')
...
const saveTitle = async () => {
  if (props.isAdmin || !editingTitle.value.trim()) return
  isSavingTitle.value = true
  try {
    await NovelAPI.updateProjectTitle(projectId, editingTitle.value.trim())
    overviewMeta.title = editingTitle.value.trim()
    await loadSection('overview', true)
    closeEditTitleModal()
    logger.info('书名修改成功')
  } catch (error) {
    logger.error('修改书名失败:', error)
    alert('修改书名失败: ' + (error instanceof Error ? error.message : '未知错误'))
  } finally {
    isSavingTitle.value = false
  }
}

const openEditFanqieIdModal = () => {
  if (props.isAdmin) return
  editingFanqieId.value = fanqieBookId.value || ''
  isEditFanqieIdModalOpen.value = true
}

const closeEditFanqieIdModal = () => {
  isEditFanqieIdModalOpen.value = false
  editingFanqieId.value = ''
}

const saveFanqieId = async () => {
  if (props.isAdmin || !editingFanqieId.value.trim()) return
  isSavingFanqieId.value = true
  try {
    await NovelAPI.updateFanqieBookId(projectId, editingFanqieId.value.trim())
    fanqieBookId.value = editingFanqieId.value.trim()
    closeEditFanqieIdModal()
    logger.info('番茄书籍ID设置成功')
  } catch (error) {
    logger.error('设置番茄书籍ID失败:', error)
    alert('设置番茄书籍ID失败: ' + (error instanceof Error ? error.message : '未知错误'))
  } finally {
    isSavingFanqieId.value = false
  }
}
