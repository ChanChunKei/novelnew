...
class Settings(BaseSettings):
    ...
    # 阿里云通义千问配置（用于混合Agent策略）
    dashscope_api_key: Optional[str] = Field(
        default=None,
        env="DASHSCOPE_API_KEY",
        description="阿里云 DashScope API Key (通义千问)"
    )
    agent_decision_model: str = Field(
        default="qwen-turbo",
        env="AGENT_DECISION_MODEL",
        description="Agent决策使用的模型（推荐qwen-turbo：便宜、RPM高）"
    )
    agent_final_model: str = Field(
        default="gpt-4o",
        env="AGENT_FINAL_MODEL",
        description="Agent最终生成使用的模型（推荐gpt-4o/claude-3-5-sonnet）"
    )
    agent_max_rounds: int = Field(
        default=5,
        ge=1,
        le=10,
        env="AGENT_MAX_ROUNDS",
        description="Agent最大决策轮次（防止无限循环）"
    )
    ...
    ollama_embedding_base_url: Optional[str] = Field(
        default=None,
    ...
    rag_provider: str = Field(
        default="libsql",
        env="RAG_PROVIDER",
        description="RAG 检索提供方：libsql（本地向量库）、gemini（Google托管）或 siliconflow（本地向量库 + SiliconFlow 嵌入）"
    )
    ...
    def _normalize_rag_provider(cls, value: Optional[str]) -> str:
        ...
        if candidate not in {"libsql", "gemini", "siliconflow"}:
            raise ValueError("RAG_PROVIDER 仅支持 libsql、gemini 或 siliconflow")
        return candidate

    @validator("embedding_base_url", "ollama_embedding_base_url", "openai_base_url", pre=True)
    def _normalize_url_fields(cls, value: Optional[str]) -> Optional[str]:
        """将空字符串转换为 None，避免 URL 验证失败。"""
