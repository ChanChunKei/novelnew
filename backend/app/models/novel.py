...
class NovelProject(Base):
    ...
    fanqie_book_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, index=True, comment="番茄小说书籍ID")
...
class Volume(Base):
    ...
    # 分卷版本化数据（JSON 字段）
    characters: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True, comment="该卷的角色快照（继承+新增+修改）")
    relationships: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True, comment="该卷的人物关系快照")
    world_setting: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True, comment="该卷的世界观快照")
